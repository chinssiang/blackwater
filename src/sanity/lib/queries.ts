...
// ---------------------------------------------------------------------------
// Filtering, sorting and facets for /products/all.
//
// Deliberately scoped to that one route. /products is prerendered per locale
// and is a curated shop window, so it reads no searchParams; putting the filter
// UI there would opt it out of static generation and undo the payload cut its
// 8-card grid exists for.
// ---------------------------------------------------------------------------

// Per-dimension predicates. Each is a no-op when its param array is empty
// (count == 0), so an unfiltered request returns the full catalogue.
// Categories/brands match on slug; badges match on value.
const productCategoryPredicate =
	`(count($categories) == 0 || count(categories[@->slug.current in $categories]) > 0)` as const;
const productBrandPredicate =
	`(count($brands) == 0 || count(brands[@->slug.current in $brands]) > 0)` as const;
const productBadgePredicate =
	`(count($badges) == 0 || count(badge[@ in $badges]) > 0)` as const;

// priceAmount, not the `price` string and not Shopify: this runs in GROQ, which
// can reach neither. It is the Sanity-side sort/filter key only — the price a
// shopper SEES on a linked product still comes from Shopify via applyCardPrices,
// so the two can drift if an editor updates one and not the other. See the field
...
const productPricePredicate =
	`(count($priceBuckets) == 0 || (defined(priceAmount) && ${productPriceBucket} in $priceBuckets))` as const;

// All dimensions AND-ed.
...
// existed. Products with no priceAmount sort last either way.
const productSortOrder = `order(
	...
	select($sort == "za" => ${productTitleOrder}) desc,
	${productTitleOrder} asc
)` as const;
...
const productFilterFacets = `
	...
	"facetBrands": *[_type == "pBrand" && count(*[_type == "pProduct" && references(^._id) && ${titleVisible}]) > 0] | order(title asc) {
		"value": slug.current,
		"label": title,
		"count": count(*[_type == "pProduct" && references(^._id) && ${titleVisible} && ${productCategoryPredicate} && ${productBadgePredicate} && ${productPricePredicate}])
	},
	"badgeCounts": {
		"founders-pick": {
