...
- `<LocationCurrentTime>` — Displays live local Taipei time on `/events*`. **Always import it from `@/components/LocationCurrentTimeLazy`**, never from the component file directly: it carries `date-fns` plus both locale bundles, and both render sites (Header, MobileMenu) sit in the always-mounted chrome — one static import anywhere puts that weight back into every route's shared chunk (measured: −61KB with both sites lazy).
...
- `src/components/ui/` — Radix UI-based: Accordion, Badge, Button, Carousel (embla, not Radix — the product gallery is its only consumer), Checkbox, Dialog, Field, Input, Label, Pagination, RadioGroup, Select, Separator, Spinner, Table, Tabs, Textarea, Tooltip.

### Utilities (`src/lib/`)

- `utils.ts` — `cn()` (Tailwind merge), `hasArrayValue()`, format helpers (`formatUrl`, `formatObjectToHtml`), `validateEmail`, outbound-link helpers (`REFERRAL_SOURCE`, `appendReferralParams()` — UTM params on `purchaseLink`), DOM helpers (`scrollDisable`, `scrollEnable`), `slugify`, `isValidUrl`, `getSpacingClass`.
- `image-utils.ts` — `buildImageSrc()`, `buildRgbaCssString()`, `ensureAccessibleTextColor()`.
- `routes.ts` — `DOCUMENT_ROUTES`, `resolveHref()`, `resolvedHrefGroq`, `checkIfLinkIsActive()`.
...
- `CartDrawer` follows `MobileMenu`'s raw Radix `Dialog` + Motion idiom (`z-popover`, `scrollDisable`/`scrollEnable`), **not** a Radix `Sheet` (that unused wrapper has since been deleted). Its width is an explicit `max-w-[26rem]` because `globals.css` remaps Tailwind's container scale (`sm` is 600px here). Checkout is an `<a>`, never a form — the site's `form-action 'self'` CSP would block a cross-origin submit.
