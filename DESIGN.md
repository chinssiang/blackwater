...
**The exception is any module whose artwork spans the section** — `heroBlock` (`src/components/HeroBlock.tsx`) and `editorialBlock` (`src/components/EditorialBlock.tsx`) — which applies `maxWidth` to a CHILD rather than to its `<section>`. The hero's background image and `<HeroWave>` are `absolute inset-0` children of the section, and an editorial image runs to the window edge in both its layouts, so capping the section capped the ARTWORK — and the copy carried its own 672px measure on top, which left the three widest rungs changing nothing visible at all. So each hands `<SectionShell>` a `maxWidth: 'none'` copy of the appearance and caps its copy column instead, through the one helper `resolveCopyColumnClass` in `src/lib/section-appearance.ts`. **This upholds the inset rule above rather than breaking it** — the cap moves off the padded box onto a child, so `p-x-max` and a `max-w-*` never meet, and `--section-inset` stays the centring form on every hero. Three consequences. The copy column needs an explicit `w-full`: the section is `flex flex-col`, and an auto cross-axis margin disables `align-self: stretch`, so without it the box is shrink-to-fit and the cap binds only once the content already exceeds it — measured on a heading-only hero, 168px against the authored 768px. Its auto margin follows the authored text alignment, for these modules only, because only here is there a narrow column inside a full-width box for the question to arise. And an authored Background Color paints full-bleed on a hero while the same field on a capped `faqBlock` is a 768px band — required by the wave, whose `mask-b-from-60%` dissolves into that paper and would otherwise meet a hard edge. `Full` keeps a reading measure rather than going edge to edge (672px on a hero, 576px on an editorial section), since every module carries `maxWidth: 'none'` from the shared `initialValue`; the cost is a dropdown that reads out of order, stated in each module's own field description.
...
| `--h-announcement` / `--height-announcement`               | **Live.** Written by `src/components/layout/Announcement/index.tsx` (a ResizeObserver) and defaulted to one line by `:root:has([data-announcement])` in `globals.css`, so a prerender already clears the bar. The bar and header are separate fixed elements, each with its own background. Everything that clears the chrome adds it: the header's own `top`, `<main>`'s padding, the `*-header-space-*` utilities and `top-header` (`--spacing-header`), which sticky bars use.                   |
