...
**The one exception is `heroBlock`** (`src/components/HeroBlock.tsx`), which applies `maxWidth` to a CHILD rather than to its `<section>`. It is the only module with a full-bleed background layer: the background image and `<HeroWave>` are `absolute inset-0` children of the section, so capping the section capped the ARTWORK — and the copy carried its own 672px measure on top, which left the three widest rungs changing nothing visible at all. So the hero hands `<SectionShell>` a `maxWidth: 'none'` copy of the appearance and caps its copy column instead. **This upholds the inset rule above rather than breaking it** — the cap moves off the padded box onto a child, so `p-x-max` and a `max-w-*` never meet, and `--section-inset` stays the centring form on every hero. Three consequences. The copy column needs an explicit `w-full`: the section is `flex flex-col`, and an auto cross-axis margin disables `align-self: stretch`, so without it the box is shrink-to-fit and the cap binds only once the content already exceeds it — measured on a heading-only hero, 168px against the authored 768px. Its auto margin follows the authored text alignment, hero-only, because only here is there a narrow column inside a full-width box for the question to arise. And an authored Background Color paints full-bleed on a hero while the same field on a capped `faqBlock` is a 768px band — required by the wave, whose `mask-b-from-60%` dissolves into that paper and would otherwise meet a hard edge. `Full` keeps the copy's 672px measure rather than going edge to edge, since every hero carries `maxWidth: 'none'` from the shared `initialValue`; the cost is a dropdown that reads out of order for a hero, stated in `hero-block.ts`'s own field description.
